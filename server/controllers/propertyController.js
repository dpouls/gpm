//ALl methods work with new db schema June 2021
module.exports = {
  getAllProperties: (req, res) => {
    if (req.session.user.is_landlord) {
      const db = req.app.get("db");
      db.property.get_all_properties().then((properties) => {
  ...
  createProperty: async (req, res) => {
    if (req.session.user.is_landlord) {
      ...
      db.property
        .insert_landlord_property(newPropertyId, landlordId)
        .then((data) =>
          res.status(200).send("Property created and connected to landlord")
        )
        .catch((err) => {
          res.send(err);
          console.log(err);
  ...
  linkPropertyRenter: (req, res) => {
    if (req.session.user.is_landlord) {
      ...
      const db = req.app.get("db");
      db.property
        .link_property_renter(renterId, propertyId)
